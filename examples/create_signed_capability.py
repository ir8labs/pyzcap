...
from zcap.models import (
    Capability,  # Assuming Capability model might be useful for type hinting or direct use
)
...
async def main():
    ...
    console.print(
        Panel("[bold cyan]1. Setup: Generating Keys and Defining DIDs[/bold cyan]")
    )
    ...
    console.print(
        Panel("[bold cyan]3. Creating and Signing the Capability[/bold cyan]")
    )
    ...
    try:
        ...
        console.print(
            Panel(
                "[bold cyan]4. Displaying the Signed Capability (JSON-LD)[/bold cyan]"
            )
        )
        ...
        if new_capability.proof:
            console.print(
                f"Proof Value (Signature): [bold yellow]{new_capability.proof.proof_value}[/bold yellow]"
            )

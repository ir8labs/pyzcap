...
async def main():
    ...
    try:
        ...
        capability_store[root_capability.id] = root_capability # Client stores the capability
    ...
    root_cap_table.add_row("Expires", str(root_capability.expires.isoformat() if root_capability.expires else 'None'))
    ...
    try:
        ...
        capability_store[delegated_capability.id] = delegated_capability # Client stores delegated cap
        console.print(f"[green]✓[/green] Delegated capability created: {delegated_capability.id}")
    except (DelegationError, CapabilityVerificationError, DIDKeyNotFoundError, CapabilityNotFoundError) as e:
    ...
    delegated_cap_table.add_row("Expires", str(delegated_capability.expires.isoformat() if delegated_capability.expires else 'None'))
    ...
    try:
        ...
        console.print("[green]✓[/green] Charlie's read invocation: [bold green]Successful attempt[/bold green]")
    ...
    if invocation:
        ...
        except (InvocationVerificationError, CapabilityVerificationError, DIDKeyNotFoundError, CapabilityNotFoundError, ZCAPException) as e:
            console.print(f"[red]✗[/red] Invocation verification failed: {e}")
    else:
        console.print("[yellow]![/yellow] Skipping invocation verification as invocation failed or was not created.")
    ...
    console.print(f"[yellow]![/yellow] Capability {delegated_capability.id} has been added to revocation list.")
    console.print()
    ...
    try:
        ...
        console.print(f"[red]✗[/red] Charlie's read after revocation: Attempt returned an invocation object (ID: {invocation_after_revoke['id'] if invocation_after_revoke else 'N/A'}). Expected failure.")
    except InvocationError as e:
        console.print(f"[green]✓[/green] Charlie's read after revocation: Failed as expected. Reason: {e}")
    except ZCAPException as e: # Catch other potential ZCAP errors
        console.print(f"[red]✗[/red] Charlie's read after revocation failed with an unexpected error: {e}")
